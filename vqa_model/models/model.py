...
class VQAGAP_qbert_Model(nn.Module):
    def __init__(self, vocab_questions, vocab_answers, input_size = 512):
        ...
        config = VisualBertConfig(num_hidden_layers=12, 
                              visual_embedding_dim=3840)
    ...
    def forward(self, input_v, input_q):
        ...
        x_q = self.tokenizer(self.vocab_questions, return_tensors='pt', padding='max_length', max_length=self.sequence_length)
        ...
        B,N = x_v.shape
        x_v = x_v.view(B,1,N)
        visual_attention_mask = torch.ones(x_v.shape[:-1], dtype=torch.long)
        visual_token_type_ids = torch.ones(x_v.shape[:-1], dtype=torch.long)
